...
public class TDTDReader extends DTDHandlerBase {
	...
	public static Model parse(InputSource dtd, InputSource bindingInfo, ErrorReceiver errorReceiver, Options opts) {
		...
		try {
			...
			try {
				...
				Model model = new Model(opts, cm, NameConverterProvider.getStandard(), opts.classNameAllocator, null);
				...
				try {
					...
					return null; // this error was already handled by
	 ...
	 * 
	 * <p>
	 * This is always non-null even if no binding information was specified. (In
	 * that case, a dummy object will be provided.)
	 */
	...
	public void endDTD() throws SAXException {
		...
		// bind them all.
		// we need to know how elements are referencing each other before we do
		// this,
	...
	private void processInterfaceDeclarations() {
		...
		for (BIInterface decl : bindInfo.interfaces()) {
			final JDefinedClass intf = classFactory.createInterface(bindInfo.getTargetPackage(), decl.name(), copyLocator());
			decls.put(decl, intf);
		...
		for (Map.Entry<BIInterface, JClass> e : decls.entrySet()) {
			...
			for (String member : decl.members()) {
				...
				if (acc == null) {
					...
					// TODO: error location
					error(decl.getSourceLocation(), Messages.ERR_BINDINFO_NON_EXISTENT_INTERFACE_MEMBER, member);
					continue;
				}
	 ...
	 * 
	 * <p>
	 * Also checks that the binding file does not contain declarations for
	...
	@Override
	public void attributeDecl(String elementName, String attributeName, String attributeType, String[] enumeration, short attributeUse,
			String defaultValue) throws SAXException {
		getOrCreateElement(elementName).attributes
				.add(createAttribute(elementName, attributeName, attributeType, enumeration, attributeUse, defaultValue));
	}

	protected CPropertyInfo createAttribute(String elementName, String attributeName, String attributeType, String[] enums, short attributeUse,
			String defaultValue) throws SAXException {
		...
		CPropertyInfo r = new CAttributePropertyInfo(propName, null, null/* TODO */, copyLocator(), qname, use, null, required);
	...
	/**
	 * Mutable {@link Locator} instance that points to the current source line.
	 * <p>
