 ...
 * <p>
 * Since unmarshaller just adds new values into the storage, we can't fill the
 * storage by default values at the time of instanciation. (or oherwise values
 * found in the document will be appended to default values, where it should
 * overwrite them.)
 * <p>
 * Therefore, when the object is created, the storage will be empty. When the
 * getXXX method is called, we'll check if the storage is modified in anyway. If
 * it is modified, it must mean that the values are found in the document, so we
 * just return it.
 * 
 * Otherwise we will fill in default values and return it to the user.
 * 
 * <p>
 * When a list has default values, its dirty flag is set to true. Marshaller
 * will check this and treat it appropriately.
 * 
 * 
...
public class NoExtendedContentField extends AbstractListField {
	...
	/**
	 * A concrete class that implements the List interface. An instance of this
	 * class will be used to store data for this field.
	 */
	...
	/**
	 * @param coreList
	 *            A concrete class that implements the List interface. An
	 *            instance of this class will be used to store data for this
	 *            field.
	 */
	protected NoExtendedContentField(ClassOutlineImpl context, CPropertyInfo prop, JClass coreList) {
		...
		// otherwise we need to do eager allocation so that the collection type
	...
	public void generateAccessors() {
		...
		String pname = NameConverterProvider.getStandard().toVariableName(prop.getName(true));
		writer.javadoc()
				.append("Gets the value of the " + pname + " property.\n\n" + "<p>\n" + "This accessor method returns a reference to the live list,\n"
	...
	class Accessor extends AbstractListField.Accessor {
		...
		public void toRawValue(JBlock block, JVar $var) {
			...
			// list.toArray( array );
			block.assign($var, JExpr._new(codeModel.ref(ArrayList.class).narrow(getType(Aspect.EXPOSED).boxify())).arg($target.invoke($get)));
		}
