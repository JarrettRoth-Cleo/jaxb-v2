 ...
 * 
 * <p>
 * One of the information contained in a global customization is the default
 * binding for properties. This object contains a BIProperty object to keep this
 * information.
 * 
...
public final class BIGlobalBinding extends AbstractDeclarationImpl {
	...
	/**
	 * Gets the name converter that will govern the XML->Java name conversion
	 * process for this compilation.
	 *
	 * <p>
	 * The "underscoreBinding" customization will determine the exact object
	 * returned from this method. The rest of XJC should just use the
	 ...
	 */
	@XmlTransient
	public NameConverter nameConverter = NameConverterProvider.getStandard();
	...
	private static enum UnderscoreBinding {
		@XmlEnumValue("asWordSeparator") WORD_SEPARATOR(NameConverterProvider.getStandard()), @XmlEnumValue("asCharInWord") CHAR_IN_WORD(
				NameConverterProvider.getJaxrpcCompatible());
	 ...
	 *
	 * This method is intended to be called by other BIXXX classes. The effect
	 * of this switch should be hidden inside this package. IOW, the
	 * reader.xmlschema package shouldn't be aware of this switch.
	 */
	...
	/**
	 * Returns true if the compiler needs to generate type-safe enum member
	 * names when enumeration values cannot be used as constant names.
	 */
	...
	/**
	 * Set of datatype names. For a type-safe enum class to be generated, the
	 * underlying XML datatype must be derived from one of the types in this
	 * set.
	 */
	...
	/**
	 * Returns {@link BISerializable} if the extension is specified, or null
	 * otherwise.
	 */
	@XmlElement
	...
	/**
	 * If &lt;xjc:superClass> extension is specified, returns the specified root
	...
	/**
	 * If &lt;xjc:superInterface> extension is specified, returns the specified
	 * root class. Otherwise null.
	 */
	@XmlElement(namespace = Const.XJC_EXTENSION_URI)
	ClassNameBean superInterface = null;

	/**
	 * Generate the simpler optimized code, but not necessarily conforming to
	 * the spec.
	 */
	...
	/**
	 * Default cap to the number of constants in the enum. We won't attempt to
	 * produce a type-safe enum by default if there are more enumeration facets
	...
	/**
	 * If true, interfaces/classes that are normally generated as a nested
	 * interface/class will be generated into the package, allowing the
	 * generated classes to be flat.
	 *
	 * See <a href="http://monaco.sfbay/detail.jsf?cr=4969415">Bug 4969415</a>
	 * for the motivation.
	 */
	...
	/**
	 * Another 1.0 compatibility customization (but we accept it and treat it as
	 * {@link #serializable})
	 */
	...
	public void setParent(BindInfo parent) {
		...
		this.defaultProperty = new BIProperty(getLocation(), null, null, null, collectionType, fixedAttributeAsConstantProperty, optionalProperty,
				generateElementProperty);
		defaultProperty.setParent(parent); // don't forget to initialize the
											// defaultProperty
	}
	...
	public void dispatchGlobalConversions(XSSchemaSet schema) {
		...
		for (Map.Entry<QName, BIConversion> e : globalConversions.entrySet()) {
			...
			if (st == null) {
				Ring.get(ErrorReceiver.class).error(getLocation(), Messages.ERR_UNDEFINED_SIMPLE_TYPE.format(name));
				continue; // abort
			}
	...
	/**
	 * Checks if the given XML Schema built-in type can be mapped to a type-safe
	 * enum class.
	 * 
	 * @param typeName
	 */
	...
	/**
	 * Used to unmarshal <xmp> <[element] name="className" /> </xmp>
	 */
	...
	/*
	 * don't want to override equals to avoid overriding hashcode for this
	 * complex object, too
	 */
	public boolean isEqual(BIGlobalBinding b) {
		boolean equal = this.isJavaNamingConventionEnabled == b.isJavaNamingConventionEnabled
				...
				&& this.generateEnumMemberName == b.generateEnumMemberName && this.codeGenerationStrategy == b.codeGenerationStrategy
				&& this.serializable == b.serializable && this.superClass == b.superClass && this.superInterface == b.superInterface
				&& this.generateElementClass == b.generateElementClass && this.generateMixedExtensions == b.generateMixedExtensions
				&& this.generateElementProperty == b.generateElementProperty && this.choiceContentProperty == b.choiceContentProperty
				&& this.optionalProperty == b.optionalProperty && this.defaultEnumMemberSizeCap == b.defaultEnumMemberSizeCap
				&& this.flattenClasses == b.flattenClasses;

		if (!equal)
			return false;

		return isEqual(this.nameConverter, b.nameConverter) && isEqual(this.noMarshaller, b.noMarshaller)
				&& isEqual(this.noUnmarshaller, b.noUnmarshaller) && isEqual(this.noValidator, b.noValidator)
				&& isEqual(this.noValidatingUnmarshaller, b.noValidatingUnmarshaller) && isEqual(this.typeSubstitution, b.typeSubstitution)
				&& isEqual(this.simpleMode, b.simpleMode) && isEqual(this.enumBaseTypes, b.enumBaseTypes)
				&& isEqual(this.treatRestrictionLikeNewType, b.treatRestrictionLikeNewType) && isEqual(this.globalConversions, b.globalConversions);
	}
