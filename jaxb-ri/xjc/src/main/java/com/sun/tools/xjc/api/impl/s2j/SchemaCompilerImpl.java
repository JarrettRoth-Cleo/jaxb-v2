 ...
 *
 * This class builds a {@link DOMForest} until the {@link #bind()} method, then
 * this method does the rest of the hard work.
 *
 * @see ModelLoader
 *
...
public final class SchemaCompilerImpl extends ErrorReceiver implements SchemaCompiler {
	...
	/**
	 * User-specified error receiver. This field can be null, in which case
	 * errors need to be discarded.
	 */
	...
	public void parseSchema(String systemId, Element element) {
		...
		try {
			...
			// it still doesn't handle xml:base and XInclude and all those
			// things
			// correctly. There's just no way to make all those things work with
			// DOM!
	...
	private void checkAbsoluteness(String systemId) {
		...
		// but OTOH we also need to be able to process system IDs like "file://a
		// b c/def.xsd",
	...
	public JAXBModelImpl bind() {
		...
		// this also takes care of the binding files given in the -episode
		// option.
		...
		try {
			...
			if (hadError)
				return null; // if we have any error by now, abort
	...
	/**
	 * We use JAXP 1.3 to do a schema correctness check, but we know it doesn't
	 * always work. So in case some people hit the problem, this switch is here
	 * so that they can turn it off as a workaround.
	 */
