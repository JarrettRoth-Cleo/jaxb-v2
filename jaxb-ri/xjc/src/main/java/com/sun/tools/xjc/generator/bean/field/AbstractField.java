 ...
 * 
 * <p>
 * This class just provides a few utility methods and keep some important
 * variables so that they can be readily accessed any time.
 *
...
abstract class AbstractField implements FieldOutline {
	...
	/**
	 * The publicly visible type of this field. If we are generating value
	 * classes implType==exposedType.
	 */
	...
	private void annotateElement(JAnnotatable field) {
		...
		// XmlElementWrapperWriter xcw =
		// field.annotate2(XmlElementWrapperWriter.class);
	 ...
	 * @param jtype
	 * @param checkWrapper
	 *            true if the method might need to generate XmlElements
	 */
	...
	private void annotateAttribute(JAnnotatable field) {
		...
		if (!generatedName.equals(ap.getName(false)) || !generatedName.equals(ap.getName(true))
				|| (outline.parent().getModel().getNameConverter() != NameConverterProvider.getStandard())) {
