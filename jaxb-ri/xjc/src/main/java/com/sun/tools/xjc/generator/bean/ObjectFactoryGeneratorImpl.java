 ...
 *
 * <p>
 * The ObjectFactory contains factory methods for each schema derived content
 * class
 *
...
abstract class ObjectFactoryGeneratorImpl extends ObjectFactoryGenerator {
	...
	/**
	 * Names of the element factory methods that are created. Used to detect
	 * collisions.
	 *
	...
	/**
	 * Names of the value factory methods that are created. Used to detect
	 * collisions.
	 *
	 * The value is used for reporting error locations.
	 */
	...
	public ObjectFactoryGeneratorImpl(BeanGenerator outline, Model model, JPackage targetPackage) {
		...
		objectFactory = this.outline.getClassFactory().createClass(targetPackage, "ObjectFactory", null);
		...
		m1.javadoc().append("Create a new ObjectFactory that can be used to " + "create new instances of schema derived classes " + "for package: "
				+ targetPackage.name());

		// add some class javadoc
		objectFactory.javadoc()
				.append("This object contains factory methods for each \n" + "Java content interface and Java element interface \n"
	...
	protected final void populate(CElementInfo ei, Aspect impl, Aspect exposed) {
		...
		{// collision check
			...
			if (existing != null) {
				outline.getErrorReceiver().error(existing.getLocator(), Messages.OBJECT_FACTORY_CONFLICT.format(ei.getSqueezedName()));
				outline.getErrorReceiver().error(ei.getLocator(), Messages.OBJECT_FACTORY_CONFLICT_RELATED.format());
				return;
			}
		}
		...
		// we sometimes have to produce (Class)List.class instead of just
		// List.class
	...
	/**
	 * return a JFieldVar that represents the QName field for the given
	 * information.
	 *
	 * if it doesn't exist, create a static field in the class and store a new
	 * JFieldVar.
	 */
	private JExpression getQNameInvocation(CElementInfo ei) {
		...
		JFieldVar qnameField = objectFactory.field(JMod.PRIVATE | JMod.STATIC | JMod.FINAL, QName.class, '_' + ei.getSqueezedName() + "_QNAME",
				createQName(name));
