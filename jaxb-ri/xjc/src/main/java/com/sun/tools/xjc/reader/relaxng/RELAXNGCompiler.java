...
public final class RELAXNGCompiler {
	 ...
	 *
	 * <p>
	 * The value is an array because we map elements with finite names to
	 * multiple classes.
	 *
	 * TODO: depending on the type of the key, the type of the values can be
	 * further restricted. Make this into its own class to represent those
	 * constraints better.
	 */
	...
	public RELAXNGCompiler(DPattern grammar, JCodeModel codeModel, Options opts) {
		...
		this.opts = opts;
		this.model = new Model(opts, codeModel, NameConverterProvider.getSmart(), opts.classNameAllocator, null);
			...
			if (p instanceof DChoicePattern) {
				...
				for (DPattern child : cp) {
					if (child instanceof DValuePattern) {
						...
						members.add(new CEnumConstant(model.getNameConverter().toConstantName(c.getValue()), null, c.getValue(), null, null/* TODO */,
								c.getLocation()));
					} else
				...
				CEnumLeafInfo xducer = new CEnumLeafInfo(model, null, new CClassInfoParent.Package(pkg), def.getName(), base,
						new ArrayList<CEnumConstant>(members), null, null/* TODO */, cp.getLocation());
	...
	private void mapToClass(DElementPattern p) {
		...
		for (QName n : names) {
			...
			bindQueue.put(types[i++] = new CClassInfo(model, pkg, name, p.getLocation(), null, n, null, null/* TODO */), p.getChild());
		}
	...
	/**
	 * Looks for named patterns that are not bound to classes so far, but that
