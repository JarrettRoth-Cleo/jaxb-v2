...
public abstract class CPropertyInfo implements PropertyInfo<NType, NClass>, CCustomizable {
	...
	/**
	 * If the base type of the property is overriden, this field is set to
	 * non-null.
	 */
	 ...
	 *
	 * If {@link #isCollection} is true, this field is always null, for we don't
	 * handle default values for a list.
	 */
	...
	protected CPropertyInfo(String name, boolean collection, XSComponent source, CCustomizations customizations, Locator locator) {
		...
		if (m != null) {
			...
			n = NameConverterProvider.getStandard().toVariableName(name);
		}

		if (!JJavaName.isJavaIdentifier(n))
			n = '_' + n; // avoid colliding with the reserved names like
							// 'abstract'.
		this.privateName = n;
	...
	/**
	 * If this model object is built from XML Schema, this property returns a
	 * schema component from which the model is built.
	 *
	 * @return null if the model is built from sources other than XML Schema
	 *         (such as DTD.)
	 */
	@Override
	 ...
	 * {@link PropertyInfo#getName()}, and therefore it always returns the name
	 * of the annotated field.
	 * <p>
	 * This name is normally not useful for the rest of XJC, which usually wants
	 * to access the "public name" of the property. A "public name" of the
	 * property is a name like "FooBar" which is used as a seed for generating
	 * the accessor methods. This is the name controlled by the schema
	 * customization via users.
	 *
	 * <p>
	 * If the caller is calling this method statically, it's usually the sign of
	 * a mistake. Use {@link #getName(boolean)} method instead, which forces you
	 * to think about which name you want to get.
	 *
	 * @deprecated marked as deprecated so that we can spot the use of this
	 *             method.
	 * 
	 * @see #getName(boolean)
	 */
	 ...
	 *
	 *            <p>
	 *            if false, this method returns the "name of the property" as
	 *            defined in the j2s side of the spec. This name is usually
	 ...
	 * {@link Plugin#postProcessModel(Model, ErrorHandler)}. But the caller
	 * should do so with the understanding that this is inherently dangerous
	 * method.
	 */
	 ...
	 *
	 * <p>
	 * In general, a property often has to be capable of representing null to
	 * indicate the absence of the value. This requires properties to be
	 * generated as <tt>@XmlElement Float f</tt>, not as
	 * <tt>@XmlElement float f;</tt>. But this is slow.
	 *
	 * <p>
	 * Fortunately, there are cases where we know that the property can never
	 * legally be absent. When this condition holds we can generate the
	 * optimized "unboxed form".
	 *
	 * <p>
	 * The exact such conditions depend on the kind of properties, so refer to
	 * the implementation code for the details.
	 *
	 * <p>
	 * This method returns true when the property can be generated as
	 * "unboxed form", false otherwise.
	 *
	 * <p>
	 * When this property is a collection, this method returns true if items in
	 * the collection is unboxable. Obviously, the collection itself is always a
	 * reference type.
	 */
	...
	/**
	 * Returns true if this property needs to represent null just for the
	 * purpose of representing an absence of the property.
	 */
	...
	/**
	 * Checks if the given {@link TypeUse} would need an explicit
	 * {@link XmlSchemaType} annotation with the given type name.
	 */
	protected static boolean needsExplicitTypeName(TypeUse type, QName typeName) {
		...
		if (itemType == null)
			// this is somewhat strange case, as it means the bound type is
			// anonymous
	...
	/**
	 * Puts the element names that this property possesses to the map, so that
	 * we can find two properties that own the same element name, which is an
	 * error.
	 *
	 * @return null if no conflict was found. Otherwise return the QName that
	 *         has the collision.
	 */
