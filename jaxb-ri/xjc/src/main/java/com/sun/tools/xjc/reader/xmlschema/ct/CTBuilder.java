 ...
 *
 * Depending on a "kind" of complex type, the binding is quite different. For
 * example, how a complex type is bound when it is extended from another complex
 * type is very different from how it's bound when it has, say, mixed content
 * model.
 *
 * Each different algorithm of binding a complex type is implemented as an
...
public abstract class CTBuilder {
	 ...
	 */
	public abstract boolean isApplicable(XSComplexType ct);

	/**
	 * Binds the given complex type. This method will be called only when the
	 * <code>isApplicable</code> method returns true.
	 */
	public abstract void build(XSComplexType ct);
