...
public abstract class AbstractExtendedComplexTypeBuilder extends CTBuilder {
	 ...
	 * the names used in its child elements [0] and attributes [1].
	 */
	protected final Map<XSComplexType, NameClass[]> characteristicNameClasses = new HashMap<XSComplexType, NameClass[]>();

	/**
	 * Computes a name class that represents everything in a given content
	 * model.
	 */
	...
	/**
	 * Checks if the particles/attributes defined in the type parameter collides
	 * with the name classes of anc/enc.
	 *
	...
	/**
	 * Looks for the derivation chain t_1 > t_2 > ... > t and find t_i such that
	 * t_i derives by restriction but for every j>i, t_j derives by extension.
	 *
	 * @return null If there's no such t_i or if t_i is any type.
	 */
	 ...
	 * UGLY.
	 * <p>
	 * If you have ctA extending ctB and ctB restricting ctC, our Java classes
	 * will look like CtAImpl extending CtBImpl extending CtCImpl.
	 *
	 * <p>
	 * Since a derived class unmarshaller uses the base class unmarshaller, this
	 * could potentially result in incorrect unmarshalling. We used to just
	 * reject such a case, but then we found that there are schemas that are
	 * using it.
	 *
	 * <p>
	 * One generalized observation that we reached is that if the extension is
	 * only adding new elements/attributes which has never been used in any of
	 * its base class (IOW, if none of the particle / attribute use / attribute
	 * wildcard can match the name of newly added elements/attributes) then it
	 * is safe to add them.
	 *
	 * <p>
	 * This function checks if the derivation chain to this type is not using
	 * restriction, and if it is, then checks if it is safe according to the
	 * above condition.
	 *
	 * @return false If this complex type needs to be rejected.
	 */
	protected boolean checkIfExtensionSafe(XSComplexType baseType, XSComplexType thisType) {
		...
		while (lastType != lastType.getBaseType()) {
			if (checkCollision(anc, enc, lastType)) {
				// This would normally return this value, but skipping so model
				// can be generated
				// return false;
			}

			if (lastType.getBaseType().isSimpleType()) // if the base type is a
														// simple type, there
														// won't be
	...
	/**
	 * Gets a {@link NameClass} that represents all the terms in the given
	 * content type. If t is not a particle, just return an empty name class.
	 */
	private NameClass getNameClass(XSContentType t) {
		...
		if (p == null)
			...
			return p.getTerm().apply(contentModelNameClassBuilder);
	}
