...
public class BGMBuilder extends BindingComponent {
	...
	/**
	 * True if the compiler is running in the extension mode (as opposed to the
	 * strict conformance mode.)
	 */
	public final boolean inExtensionMode;

	/**
	 * If this is non-null, this package name takes over all the schema
	 * customizations.
	 */
	public final String defaultPackage1;

	/**
	 * If this is non-null, this package name will be used when no customization
	 * is specified.
	 */
	...
	private void promoteGlobalBindings() {
		...
		for (XSSchema s : schemas.getSchemas()) {
			...
			if (globalBinding == null) {
				...
				if (!globalBinding.isEqual(gb)) { // see Issue 687 - this may
													// happen with syntactically
													// imported documents
					// acknowledge this customization and report an error
					// otherwise the user will see "customization is attached to
					// a wrong place" error,
					// which is incorrect
	 ...
	 * Global bindings.
	 *
	 * The empty global binding is set as the default, so that there will be no
	 * need to test if the value is null.
	 */
	...
	/**
	 * Name converter that implements "XML->Java name conversion" as specified
	 * in the spec.
	 *
	 * This object abstracts the detail that we use different name conversion
	 * depending on the customization.
	 *
	 * <p>
	 * This object should be used to perform any name conversion needs, instead
	 * of the JJavaName class in CodeModel.
	 */
	...
	/**
	 * Reports an error if there are more than one jaxb:schemaBindings
	 * customization.
	 */
	private void checkMultipleSchemaBindings(XSSchema schema) {
		...
		for (int i = 1; i < locations.size(); i++)
			getErrorReporter().error((Locator) locations.get(i), Messages.ERR_MULTIPLE_SCHEMA_BINDINGS_LOCATION);
	}

	/**
	 * Calls {@link ClassSelector} for each item in the iterator to populate
	 * class items if there is any.
	 */
	...
	private void processPackageJavadoc(XSSchema s) {
		...
		if (cust.getJavadoc() == null)
			return; // no javadoc customization
	 ...
	 *
	 * @return Always return a non-null valid BindInfo object. Even if no
	 *         declaration was specified, this method creates a new BindInfo so
	 *         that new decls can be added.
	 */
	 ...
	 *
	 * @return always return a valid {@link BindInfo} object. If none is
	 *         specified for the given component, a dummy empty BindInfo will be
	 *         returned.
	 */
	...
	private final XSFinder toPurple = new XSFinder() {
		...
		public Boolean wildcard(XSWildcard xsWildcard) {
			...
			// element wildcards should have been processed with particle
			// binders
			return true;
		}
	};
	...
	/**
	 * If the component maps to a property, forwards to purple, otherwise to
	 * green.
	 *
	 ...
	 *
	 * This method handles prefix/suffix modification and XML-to-Java name
	 * conversion.
	 *
	 * @param name
	 *            The base name. This should be things like element names or
	 *            type names.
	 * @param comp
	 *            The component from which the base name was taken. Used to
	 *            determine how names are modified.
	 */
