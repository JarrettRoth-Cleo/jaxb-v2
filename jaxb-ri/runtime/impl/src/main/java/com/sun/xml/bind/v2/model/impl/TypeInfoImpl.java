...
/**
 * Common implementation between {@link ClassInfoImpl} and
 * {@link ElementInfoImpl}.
 *
...
abstract class TypeInfoImpl<TypeT, ClassDeclT, FieldT, MethodT> implements TypeInfo<TypeT, ClassDeclT>, Locatable {
	...
	/**
	 * The Java class that caused this Java class to be a part of the JAXB
	 * processing.
	 *
	...
	/**
	 * Reference to the {@link ModelBuilder}, only until we link
	 * {@link TypeInfo}s all together, because we don't want to keep
	 * {@link ModelBuilder} too long.
	 */
	...
	/**
	 * Parses an {@link XmlRootElement} annotation on a class and determine the
	 * element name.
	 *
	...
	protected final QName parseElementName(ClassDeclT clazz) {
		...
		if (local.equals("##default")) {
			// if defaulted...
			local = NameConverterProvider.getStandard().toVariableName(nav().getClassShortName(clazz));
		}
	...
	/**
	 * Parses a (potentially-null) {@link XmlType} annotation on a class and
	 * determine the actual value.
	 *
	 * @param clazz
	 *            The class on which the XmlType annotation is checked.
	 * @param t
	 *            The {@link XmlType} annotation on the clazz. This value is
	 *            taken as a parameter to improve the performance for the case
	 *            where 't' is pre-computed.
	 */
